...
(function() {
  ...
  // ── 1. Block all popup/new-window attempts ──
  ...
  // Intercept addEventListener for beforeunload / unload tricks
  ...
  // ── 2. Block document.write injection ──
  document.write = function() {};
  document.writeln = function() {};

  // ── 3. Patch history API to prevent redirect loops ──
  ...
  // ── 4. Kill timers that re-spawn ads ──
  ...
  const AD_CLASSES = [
    ...
    'floating-ad','sticky-ad','leaderboard','skyscraper','rectanglead',
  ...
  const isAdEl = (el) => {
    ...
    // Catch iframes loading ad domains
    ...
    try {
      ...
      all.forEach(el => {
        ...
        if ((cs.position === 'fixed' || cs.position === 'sticky') && !isVideoEl(el)) {
          // Only remove if it's clearly an overlay (high z-index, full-width)
...
// JS injected into a hidden WebView to sniff the raw video stream URL
...
(function() {
  ...
  // ── Poll video + track elements every 600ms ──
  function checkMedia() {
    document.querySelectorAll('video').forEach(function(v) {
      if (v.src && !v.src.startsWith('blob:')) sendVideoUrl(v.src, 'poll-src');
      if (v.currentSrc && !v.currentSrc.startsWith('blob:')) sendVideoUrl(v.currentSrc, 'poll-currentSrc');
      v.querySelectorAll('source').forEach(function(s) { sendVideoUrl(s.src, 'poll-source'); });
      // Check text tracks
      try {
        var tracks = v.textTracks;
        for (var i = 0; i < (tracks ? tracks.length : 0); i++) {
          var t = tracks[i];
          // textTrack doesn't expose src directly, check children
        }
      } catch(e) {}
    });
    // Check <track> elements
    document.querySelectorAll('track').forEach(function(t) {
      sendSubtitleUrl(t.src, t.label || t.srclang || '');
    });
    // Check link/script JSON payloads for subtitle manifest data
    document.querySelectorAll('[data-tracks],[data-subtitles]').forEach(function(el) {
      var raw = el.getAttribute('data-tracks') || el.getAttribute('data-subtitles') || '';
      try {
        var arr = JSON.parse(raw);
        if (Array.isArray(arr)) {
          arr.forEach(function(item) {
            if (item.file) sendSubtitleUrl(item.file, item.label || '');
            if (item.src)  sendSubtitleUrl(item.src,  item.label || '');
          });
        }
      } catch(e) {}
    });
  ...
  new MutationObserver(checkMedia).observe(document.documentElement, {
    childList: true, subtree: true, attributes: true, attributeFilter: ['src','data-tracks','data-subtitles']
  });
