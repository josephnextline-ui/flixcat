...
export function useStreamExtraction(providerIdx, season, episode, player) {
  ...
  // Reset everything when provider/episode/season changes — also pause native player
  useEffect(() => {
    try {
      player.pause();
    } catch (e) {}
  ...
  // Handle messages from the hidden extraction WebView
    ...
    try {
      ...
      if (data.type === "VIDEO_URL" && data.url) {
        ...
        setSubtitles((prev) => {
          ...
          // Auto-select English if not already selected
